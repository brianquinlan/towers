...
class RotateableSprite {
  ...
  public RotateableSprite(int textureId, int width, int height) {
    ...
    final ByteBuffer tbb = ByteBuffer.allocateDirect(vertices.length / 3 * 2 * 4);
  ...
  static public List<RotateableSprite> loadSprites(Context context, GL10 gl, int... resourceIds) {
    final ArrayList<RotateableSprite> sprites = new ArrayList<RotateableSprite>(resourceIds.length);

    final int[] textures = new int[resourceIds.length];
    gl.glGenTextures(resourceIds.length, textures, 0);
    for (int i = 0; i < resourceIds.length; ++i) {
      final Bitmap m = Sprite.loadBitmap(context, resourceIds[i]);
