...
public class NavigationGrid {
  ...
  protected void handleCell(LinkedList<WorkItem> work, Point cell, Point sponsor) {
    if (cell.x >= 0 && cell.x < width &&
      ...
      final Cell c = grid[cell.x][cell.y];

      if (sponsor == null) {
        c.nextCell = null;
        c.totalCost = c.cost;
      } else {
        final Cell s = grid[sponsor.x][sponsor.y];
  ...
  protected void calc() {
    final LinkedList<WorkItem> work = new LinkedList<WorkItem>();
    ...
    for (final Point p : goalCells) {
      handleCell(work, new Point(p.x, p.y), null);
    }


    while (!work.isEmpty()) {
      final WorkItem w = work.poll();
